...
cdef class MCTS():
    ...
    def search(self, timemax=None, itermax=None, batch=10000, explore=1, verbose=0):
        ...
        if len(nextmoves)>1:  #unimportant -- determine when to stop
            if itermax or timemax:
                ...
                while (timemax is None or time.time()<start+timemax) and\
                    ...
                    i = i+batch
                    li=list(range(batch//cpus*100,batch//cpus*100+cpus))
                    vs,nodes = zip(*pool.map(simulate_multiple,[(self.game,self.nodes,e,explore) for e in li]))
                    v=max(vs)                    
                ...
                while True:
                    vs,nodes = zip(*pool.map(simulate_multiple,
                                             [(self.game,self.nodes,e,explore) for e in range(batch*100,batch*100+cpus)]))
                    v=max(vs)
...
def simulate_multiple(p):
    ...
    for _ in range(OneBatch+1):
        v = simulate(deepcopy(game),node,nodes,ex,1)
        if v>greatestval:greatestval=v
    return greatestval,nodes
    
cdef double simulate(game,node,nodes,double explore=1.,int mode=1):
    cdef double v
    ...
    if node['untriedMoves'] == [] and node['childNodes'] != {}: #fully expanded, non-terminal
        ...
        if mode%2==0:
            ...
            for mv,k in node['childNodes'].items():
                nn = nodes[k]
                moves += mv,
                probs += nn['wins']/nn['visits'],
            if inf in probs:
                move = moves[probs.index(inf)]
            else:
                probs=np.clip(probs,0.001,1)
                move = np.random.choice(moves,p=probs/np.sum(probs))
        else:
            for mv,k in node['childNodes'].items():
                nn = nodes[k]
        ...
        if nnode['wins']==inf or (nnode['wins']==-inf and mode%2==1): #only if selecting *the best*
            node['wins']+= -nnode['wins']; node['visits']+=1
            return -nnode['wins']
        game.Move(move)
        v = simulate(game,nnode,nodes,explore,mode+1)
